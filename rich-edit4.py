...
class App:
    ...
    def edit_module(self):
        ...
        try:
            # Read submodule index (required)
            idx_str = input("Submodule Index (0-7): ").strip()
            if not idx_str:
                return
            mod_idx = int(idx_str)

            # 1. Handle Submodule Intro (Type ID)
            # Goal: If new_typeId not 0/blank and DLC provided, send CFG_SUB_INTRO_MSG
            t_id_raw = input("New Type ID (hex, optional): ").strip()
            if t_id_raw:
                new_type_id = int(t_id_raw, 16)
                new_type_dlc = int(input("New Type DLC (max 8): ") or 0)
                # Payload: NodeID(4) + ModIdx(1) + TypeID(2) + DLC(1) = 8 bytes
                payload = struct.pack('>I B H B', target_id, mod_idx, new_type_id, new_type_dlc)
                self.bus.send(can.Message(arbitration_id=CFG_SUB_INTRO_MSG_ID, data=payload))
                self.add_log(f"Sent Config Update TypeId 0x{new_type_id:X}", target_id)

            # 2. Handle Submodule Data (Data ID)
            # Goal: If new_dataId not 0/blank and DLC provided, send CFG_SUB_DATA_MSG
            d_id_raw = input("New Data ID (hex, optional): ").strip()
            if d_id_raw:
                new_data_id = int(d_id_raw, 16)
                new_data_dlc = int(input("New Data DLC (max 8): ") or 0)
                # Payload: NodeID(4) + ModIdx(1) + DataID(2) + DLC(1) = 8 bytes
                payload = struct.pack('>I B H B', target_id, mod_idx, new_data_id, new_data_dlc)
                self.bus.send(can.Message(arbitration_id=CFG_SUB_DATA_MSG_ID, data=payload))
                self.add_log(f"Sent Config Update DataId 0x{new_data_id:X}", target_id)

            # 3. Handle Raw Data (Config Bytes)
            # Goal: If data_str not blank send CFG_SUB_RAW_DATA + 4-byte data
            d_str_raw = input("Enter 3 data bytes (hex, space separated): ").strip()
            if d_str_raw:
                parts = d_str_raw.split()
                if len(parts) >= 3: # User requested 3 values, but often 4 are used for alignment
                    b1 = int(parts[0], 16)
                    b2 = int(parts[1], 16)
                    b3 = int(parts[2], 16)
                    b4 = int(parts[3], 16) if len(parts) > 3 else 0
                    # Payload: NodeID(4) + ModIdx(1) + 3 config bytes = 8 bytes
                    payload = struct.pack('>I B B B B', target_id, mod_idx, b1, b2, b3)
                    self.bus.send(can.Message(arbitration_id=CFG_SUB_RAW_DATA_ID, data=payload))
                    self.add_log(f"Sent Config Raw Data for Index {mod_idx}", target_id)

            # Invalidate cache to trigger a re-interview of the node
            with self.state.lock:
            ...
            self.add_log(f"Input Error: {e}")
        except Exception as e:
            self.add_log(f"Edit Error: {e}")
    ...
    def persist_selected(self):
        ...
        node_ids = sorted(self.state.nodes.keys())
        if not node_ids: 
            self.interaction_active = False # Ensure flag is reset if no nodes exist
            return
        target_id = node_ids[self.selected_idx % len(node_ids)]
