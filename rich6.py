...
DATA_CONFIG_CRC_ID   = 0x420
DATA_CFGWRITE_FAILED = 0x421
...
class CANState:
    ...
    def touch(self, node_id: int):
        with self.lock:
            nd = self.nodes.setdefault(node_id, {
                ...
                'last': time.time(), 'nvs_status': "Pending"
            })
            nd['last'] = time.time()
            return nd
    ...
    def calculate_node_crc(self, node_id: int):
        ...
        # Guard: Only process up to 8 sub-modules to stay within 136 byte limit
...
class App:
        ...
        while not self.q.empty():
            ...
            if 0x700 <= arb_id <= 0x7FF:
                ...
                if node['sub_mod_cnt'] == 0:
                    ...
                    # If sub_mod_cnt is 0 or invalid, skip
                    if node['sub_mod_cnt'] == 0:
                        node['interview_complete'] = True 
                else:
                    idx, is_b = m_byte & 0x7F, bool(m_byte & 0x80)
                    # Ignore legacy nodes sending indices >= 8
                    if idx < 8:
                ...
                self.state.touch(nid)['nvs_status'] = "[bold green]SAVED[/]"
            
            elif arb_id == DATA_CFGWRITE_FAILED:
                nid = struct.unpack('>I', data[0:4])[0]
                self.state.touch(nid)['nvs_status'] = "[bold red]FAIL[/]"
    ...
    def _build_layout(self):
        ...
        table.add_column("CRC Status", width=20)
        table.add_column("NVS", width=12)

        with self.state.lock:
            for nid, n in self.state.nodes.items():
                mod_summary = []
                for i in range(min(n['sub_mod_cnt'], 8)):
                    s = n['subs'].get(i)
                    if s:
                        t_id = f"0x{s['telemetry']['id']:03X}" if s['telemetry'] else "-"
                        mod_summary.append(f"M{i}:[dim]{t_id}[/]")
                ...
                table.add_row(f"0x{nid:08X}", " ".join(mod_summary), crc_str, n['nvs_status'])

        l = Layout()
        l.split(
            Layout(Text(f" CAN Master | {self.iface} | (q)uit (b)roadcast (p)ersist", style="bold reverse blue"), size=1),
            Layout(table, name="body"),
            Layout(Panel(Text("\n".join(self.log_lines)), title="System Log", border_style="cyan"), size=10)
        )
        return l
